...
def analyze_by_experiment_type(results: List[Dict]):
    ...
    for exp_type, experiments in sorted(by_type.items()):
        ...
        for exp in experiments:
            white_agent_wins = int(exp['white_agent_wins'])
            draws = int(exp['draws'])
            black_agent_wins = int(exp['black_agent_wins'])

            win_rate = calculate_win_rate(white_agent_wins, draws, black_agent_wins)

            print(f"  {exp['experiment_id']}: {exp['description']}")
            print(f"    {exp['white_agent']} {white_agent_wins}-{draws}-{black_agent_wins} {exp['black_agent']}")
...
def analyze_head_to_head_matrix(results: List[Dict]):
    ...
    for result in h2h_results:
        if result['white_agent'] == 'mcts':
            mcts_configs.add(json.loads(result['white_config'])['simulations'])
        if result['black_agent'] == 'minimax':
            minimax_configs.add(json.loads(result['black_config'])['depth'])
    ...
    matrix = {}
    for result in h2h_results:
        mcts_sims = json.loads(result['white_config'])['simulations']
        mm_depth = json.loads(result['black_config'])['depth']

        white_agent_wins = int(result['white_agent_wins'])
        draws = int(result['draws'])
        black_agent_wins = int(result['black_agent_wins'])
...
def analyze_degradation(results: List[Dict]):
    ...
    # Group by agent type
    minimax_deg = [r for r in deg_results if r['white_agent'] == 'minimax']
    mcts_deg = [r for r in deg_results if r['white_agent'] == 'mcts']

    if minimax_deg:
        print("\nMinimax vs Greedy (varying depth):")
        print("-" * 40)
        for result in sorted(minimax_deg, key=lambda x: -int(json.loads(x['white_config'])['depth'])):
            depth = json.loads(result['white_config'])['depth']
            white_agent_wins = int(result['white_agent_wins'])
            draws = int(result['draws'])
            black_agent_wins = int(result['black_agent_wins'])
            win_rate = calculate_win_rate(white_agent_wins, draws, black_agent_wins)
            print(f"  Depth {depth}: {white_agent_wins}-{draws}-{black_agent_wins} (win rate: {win_rate:.1%})")

    if mcts_deg:
        print("\nMCTS vs Greedy (varying simulations):")
        print("-" * 40)
        for result in sorted(mcts_deg, key=lambda x: -int(json.loads(x['white_config'])['simulations'])):
            sims = json.loads(result['white_config'])['simulations']
            white_agent_wins = int(result['white_agent_wins'])
            draws = int(result['draws'])
            black_agent_wins = int(result['black_agent_wins'])
            win_rate = calculate_win_rate(white_agent_wins, draws, black_agent_wins)
            print(f"  Sims {sims:3d}: {white_agent_wins}-{draws}-{black_agent_wins} (win rate: {win_rate:.1%})")
